...
router.get("/", (req, res) => {
  try {
    ...
    if (isNewOnly) {
      whereClauses.push(
        `strftime('%m', date_ajout) = strftime('%m', 'now') AND strftime('%Y', date_ajout) = strftime('%Y', 'now')`,
      );
    }
...
router.post("/", (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res
        .status(409)
        .json({ error: "Ce médicament existe déjà dans la base de données." });
    }
...
router.post("/interactions", (req, res) => {
  const { medicamentSourceId, medicamentInterditId, riskLevel, description } =
    req.body;
  if (!medicamentSourceId || !medicamentInterditId) {
    return res
      .status(400)
      .json({ error: "Source and Interdit medication IDs are required" });
  }
  ...
  try {
    ...
    if (userRole?.nom_type !== "Administrateur") {
      return res
        .status(403)
        .json({ error: "Only Administrateurs can manage interactions" });
    }
    ...
    res
      .status(201)
      .json({ success: true, interactionId: info.lastInsertRowid });
  } catch (error) {
...
Si le médicament est inconnu, retourne uniquement: {"error":"Médicament inconnu"}`;
    ...
    const { text, error, status } = await geminiGenerateText(prompt, {
      json: true,
      maxOutputTokens: 768,
    });
    ...
    if (!text) {
      return res
        .status(status ?? 502)
        .json({ error: error ?? "Erreur API Gemini" });
    }

    try {
      const parsed = parseJsonFromAiText(text) as AiMedicationInfo;
