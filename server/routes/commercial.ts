...
router.post("/register-client", async (req, res) => {
  const { commercialId, clientPhone, clientName, prescription, startDate } =
    req.body;
  ...
  if (!commercialId || !clientPhone || !clientName || !prescription) {
    return res
      ...
      .json({
        error:
          "Tous les champs sont requis (Commercial ID, Phone, Name, Prescription)",
      });
  }
  ...
  try {
    ...
    const availability = checkClientAvailability(
      normalizedName,
      normalizedPhone,
    );
    ...
    const insertTransaction = db.transaction(() => {
      ...
      ).run(idUtilisateur, normalizedName);
...
router.post("/add-prescription", async (req, res) => {
  ...
  try {
    ...
    const client = db
      .prepare(
        "SELECT id_utilisateur FROM Utilisateurs WHERE id_utilisateur = ? AND id_createur = ?",
      )
      .get(clientId, commercialId);
    if (!client) {
      return res
        .status(403)
        .json({ error: "Accès refusé ou client non trouvé." });
    }
    ...
    const transaction = db.transaction(() => {
      const ordInfo = db
        ...
        .run(
          clientId,
          prescription.title || "Nouvelle Ordonnance",
          clientId,
      ...
      for (const m of prescription.medications) {
        let medRecord = db
          .prepare(
            "SELECT id_medicament FROM Medicaments WHERE LOWER(nom) = LOWER(?)",
          )
          .get(m.name) as { id_medicament: number } | undefined;
        let idMedicament = medRecord
          ? medRecord.id_medicament
          : db.prepare("INSERT INTO Medicaments (nom) VALUES (?)").run(m.name)
              .lastInsertRowid;
...
router.post("/validate-client", async (req, res) => {
  ...
  try {
    ...
    db.prepare(
      "UPDATE Utilisateurs SET est_valide = 1 WHERE id_utilisateur = ?",
  ...
  try {
    ...
    if (existing) {
      return res
        .status(409)
        .json({
          error: "Un client avec ce nom existe déjà dans la base de données.",
        });
    }
...
router.post("/send-message", async (req, res) => {
  ...
  if (!commercialId || !clientId || !message) {
    return res
      .status(400)
      .json({ error: "Commercial ID, Client ID et message requis" });
  }
  ...
  try {
    ...
    const client = db
      ...
      .get(clientId, numericCommercialId);
    if (!client) {
      return res
        .status(403)
        .json({ error: "Accès refusé ou client non trouvé." });
    ...
    const commercialProfile = db
      .prepare(
        "SELECT nom_complet FROM ProfilsUtilisateurs WHERE id_utilisateur = ?",
      )
      .get(numericCommercialId) as { nom_complet: string } | undefined;
    const senderName =
      commercialProfile?.nom_complet || "Votre conseiller TAKYMED";
...
router.delete("/clients/:id", (req, res) => {
  const { id } = req.params;
  const finalCommercialId =
    (req.body && req.body.commercialId) || req.query.commercialId;
  ...
  try {
    ...
    const result = db
      .prepare(
        "DELETE FROM Utilisateurs WHERE id_utilisateur = ? AND id_createur = ?",
      )
      .run(id, finalCommercialId);
    if (result.changes === 0) {
      return res
        .status(404)
        .json({ error: "Client non trouvé ou non autorisé." });
