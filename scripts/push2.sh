...
# 3. Build on Remote (disable WhatsApp QR by setting env var)
echo "🔧 Building on remote server (WhatsApp disabled)..."
$SSH_CMD $REMOTE_USER@$REMOTE_HOST "cd $REMOTE_DIR && DISABLE_WHATSAPP=1 NODE_VER=\$(node -v | cut -d. -f1 | sed 's/v//') && if [ \"$NODE_VER\" -lt 20 ]; then echo \"⚠️ Node.js version too old (\$NODE_VER). Updating to Node 22...\" && curl -fsSL https://deb.nodesource.com/setup_22.x | bash - && apt-get install -y nodejs; fi && if ! command -v pm2 >/dev/null; then echo \"📦 Installing PM2 globally...\" && npm install -g pm2; fi && DISABLE_WHATSAPP=1 npm install && DISABLE_WHATSAPP=1 npm run build" || { echo "❌ Remote build failed."; exit 1; }

# 4. Restart the application with PM2
echo "🔄 Restarting application with PM2..."
$SSH_CMD $REMOTE_USER@$REMOTE_HOST "cd $REMOTE_DIR && pm2 stop takymed-backend 2>/dev/null || true && pm2 delete takymed-backend 2>/dev/null || true && pm2 restart takymed --update-env || (PORT=$PORT pm2 start dist/server/node-build.mjs --name takymed) && pm2 save" || { echo "❌ Failed to restart application with PM2."; exit 1; }
