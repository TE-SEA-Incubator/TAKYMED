...
export default function Ordonnances() {
  ...
  async function fetchOrdonnances() {
    if (!user) return;
    try {
      const res = await fetch(`http://dev.takymed.com/api/ordonnances?userId=${user.id}`);
      if (res.ok) {
  ...
  async function fetchOrdonnanceDetails(id: number) {
    try {
      const res = await fetch(`http://dev.takymed.com/api/ordonnances/${id}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const cancelOrdonnance = async (id: number) => {
    try {
      const res = await fetch(`http://dev.takymed.com/api/ordonnances/${id}/cancel`, {
        method: "PATCH"
      });
      if (res.ok) {
  ...
  const reactivateOrdonnance = async (id: number) => {
    try {
      const res = await fetch(`http://dev.takymed.com/api/ordonnances/${id}/reactivate`, {
        method: "PATCH"
      });
      if (res.ok) {
  ...
  const saveEditMed = async (ordonnanceId: number, medId: number) => {
    try {
      const res = await fetch(`http://dev.takymed.com/api/ordonnances/${ordonnanceId}/medicaments/${medId}`, {
        method: "PUT",
        headers: authHeaders(),
  ...
  const addMedicament = async (ordonnanceId: number) => {
    ...
    try {
      const res = await fetch(`http://dev.takymed.com/api/ordonnances/${ordonnanceId}/medicaments`, {
        method: "POST",
        headers: authHeaders(),
  ...
  const updateRappelTime = async (rappelId: number, heure_prevue: string, ordonnanceId: number) => {
    try {
      const res = await fetch(`http://dev.takymed.com/api/ordonnances/prises/${rappelId}`, {
        method: "PATCH",
        headers: authHeaders(),
  ...
  const markAllPrisesTaken = async (ordonnanceId: number) => {
    try {
      const res = await fetch(`http://dev.takymed.com/api/ordonnances/${ordonnanceId}/prises/mark-all-taken`, {
        method: "PATCH",
        headers: authHeaders(),
        body: JSON.stringify({})
      });
  ...
  const delayRappel = async (rappelId: number, ordonnanceId: number) => {
    try {
      const res = await fetch(`http://dev.takymed.com/api/prescriptions/doses/${rappelId}/delay`, { method: "POST" });
      if (res.ok) {
