...
export default function Dashboard() {
  ...
  const handleChangeName = async () => {
    ...
    try {
      const res = await fetch("https://dev.takymed.comhttps://dev.takymed.com/api/auth/profile", {
        method: "PATCH",
        headers: {
...
function DashboardSecurityCard({ user }: { user: any }) {
  ...
  useEffect(() => {
    ...
    const fetchPinInfo = async () => {
      if (!user?.id) return;
      try {
        const res = await fetch("https://dev.takymed.comhttps://dev.takymed.com/api/auth/pin-info", {
          headers: {
            "x-user-id": user.id.toString(),
          },
        });
  ...
  const handleRegeneratePin = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch("https://dev.takymed.comhttps://dev.takymed.com/api/auth/regenerate-pin", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "x-user-id": user.id.toString(),
